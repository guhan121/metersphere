      ...
      <el-tabs v-model="activeName" style="margin-left: 4px">
        <el-tab-pane label="基础信息" name="BaseInfo" class="overflow-y">
          <div class="ms-main-div" @click="showAll" v-if="type!=='detail'" style="margin-top: 4px">
            ...
            <div class="ms-opt-btn">
              <el-link type="primary" style="margin-right: 20px" @click="openHis"
                       v-if="path === '/api/automation/update'">{{ $t('operating_log.change_history') }}
              </el-link>
        ...
        <el-tab-pane label="场景步骤" name="Steps" class="overflow-y">
          ...
          <div class="ms-opt-btn" style="margin-top: 4px">
            <el-button id="inputDelay1" type="primary" size="small" v-prevent-re-click @click="editScenario"
                       title="ctrl + s">
              {{ $t('commons.save') }}
            </el-button>
          </div>
                  ...
                  <el-row style="margin: 5px">
                    ...
                    <el-col :span="4">
                      <env-popover :disabled="scenarioDefinition.length < 1" :env-map="projectEnvMap"
                                   ...
                                   :project-list="projectList" ref="envPopover" v-tester/>
                    </el-col>
                    <el-col :span="3" class="ms-col-one ms-font">
                    ...
                    <el-col :span="4">
                      <el-button :disabled="scenarioDefinition.length < 1" size="mini" type="primary" v-prevent-re-click
                                 @click="runDebug" v-tester>{{ $t('api_test.request.debug') }}
                      </el-button>
                ...
                <div>
                  ...
                  <el-tree node-key="resourceId" :props="props" :data="scenarioDefinition" class="ms-tree"
                    ...
                    <span class="custom-tree-node father" slot-scope="{ node, data}" style="width: 96%">
                      <!-- 步骤组件-->
                       <ms-component-config :type="data.type" :scenario="data" :response="response"
        ...
        <el-tab-pane label="调试结果" name="Report" class="overflow-y">
          <ms-api-report-detail v-if="debugVisible" :report-id="reportIdCopy" :debug="true"
      ...
      <ms-drawer :visible="drawer" :size="100" @close="close" direction="default" :show-full-screen="false"
        ...
        <el-tabs v-model="activeName" style="margin-left: 4px">
          <el-tab-pane label="场景步骤" name="Steps">
            <maximize-scenario :scenario-definition="scenarioDefinition" :envMap="projectEnvMap"
                               :moduleOptions="moduleOptions"
                               :currentScenario="currentScenario" :type="type" ref="maximizeScenario"
